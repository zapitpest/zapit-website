...
| Adam | Meta Business Manager Pixel access (Pixel ID needed for Phase 2 tags) | 2026-06-18 + still needed | ⏳ Ask outstanding — request to be re-raised in next status email. |
| Adam | Feature Parity decisions (231 suburb URLs + 59 blog URLs + birds page) | 2026-06-27 (audit doc shared) | ⏳ Phase 3 cutover gate — not blocking Phase 2. Focused decision email drafted in `docs/FEATURE_PARITY_AUDIT.md`. |
...
| Adam | Hours top-up (current runway ~3 hr; Phase 2 needs ~6-7 hr) | To be flagged | ⏳ Will raise in next Friday status email. |
